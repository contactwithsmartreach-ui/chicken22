...
export const ExploreGallery = () => {
  ...
  return (
    <section id="gallery" className="py-32 relative overflow-hidden bg-gradient-to-b from-[#CDFAF8] via-[#a2f5f1] to-[#CDFAF8]">
      {/* Decorative gradient glowing blur elements */}
      <div className="absolute top-1/4 left-[-10%] w-[500px] h-[500px] bg-teal-300/30 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-[-10%] w-[500px] h-[500px] bg-emerald-200/30 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-16 mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8 relative z-10">
        <div>
          <h2 className="text-5xl sm:text-7xl font-serif font-light tracking-tight text-neutral-900 leading-none">
            Explore <span className="italic font-normal text-teal-800">Creations</span>
          </h2>
        </div>
        <p className="text-neutral-700 max-w-sm text-sm leading-relaxed font-medium">
          Select a category below to filter masterworks and inspect their culinary profile.
        </p>
      </div>

      {/* Brand Scroller Ticker Integration */}
      <div className="mb-16 space-y-4 py-4 border-y border-teal-900/10 bg-white/20 backdrop-blur-md relative z-10">
        <BrandScroller />
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-xs font-semibold tracking-wider uppercase transition-all duration-300 flex-shrink-0 border backdrop-blur-md ${
                selectedCategory === cat
                  ? "bg-neutral-900 text-teal-100 border-neutral-900 shadow-xl shadow-teal-900/10"
                  : "bg-white/40 text-neutral-800 border-white/60 hover:bg-white/60 hover:border-white"
              }`}
            >
              {cat}
            </button>
      ...
      >
        {filteredDishes.map((dish, index) => (
          <div
            ...
            className="group relative flex-shrink-0 w-[340px] sm:w-[420px] rounded-3xl overflow-hidden bg-white/40 backdrop-blur-xl border border-white/60 cursor-pointer snap-center transition-all duration-500 hover:border-teal-500/50 hover:bg-white/60 hover:shadow-2xl hover:shadow-teal-900/10 transform hover:-translate-y-2"
          >
            <div className="relative h-80 overflow-hidden">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-neutral-950/70 via-neutral-950/10 to-transparent opacity-80" />

              <div className="absolute bottom-4 left-6 font-mono text-xs text-teal-200">
                0{index + 1} / 0{filteredDishes.length}
              </div>
            </div>

            <div className="p-8">
              <h3 className="font-serif text-2xl font-medium text-neutral-900 mb-3 group-hover:text-teal-900 transition-colors">
                {dish.name}
              </h3>
              <p className="text-neutral-700 text-sm line-clamp-2 leading-relaxed mb-6 font-normal">
                {dish.description}
              </p>
              <div className="flex items-center justify-between pt-4 border-t border-neutral-900/10 text-xs text-neutral-700">
                <span className="tracking-wider uppercase font-medium">{dish.origin.split("&")[0]}</span>
                <span className="text-teal-900 font-semibold tracking-wide flex items-center gap-1 group-hover:translate-x-1 transition-transform">
      ...
      {activeModalDish && (
        <div className="fixed inset-0 z-50 bg-neutral-950/60 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
          <div className="relative w-full max-w-3xl bg-white/90 backdrop-blur-2xl border border-white/85 rounded-3xl overflow-hidden shadow-2xl">
            <button
              onClick={() => setActiveModalDish(null)}
              className="absolute top-6 right-6 z-20 w-10 h-10 rounded-full bg-neutral-900/10 hover:bg-neutral-900/20 text-neutral-900 flex items-center justify-center transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="p-8 sm:p-10 flex flex-col justify-between">
                <div className="space-y-4">
                  <span className="px-3 py-1 rounded-full bg-teal-900/10 text-teal-900 text-xs font-semibold tracking-wider uppercase border border-teal-900/20">
                  ...
                  <div className="pt-4 space-y-3 border-t border-neutral-900/10">
                    <div>
                      <span className="text-xs uppercase tracking-wider text-neutral-500 block mb-1">Sourcing Origin</span>
                      <span className="text-sm text-neutral-900 font-medium">{activeModalDish.origin}</span>
                    </div>
                    <div>
                      <span className="text-xs uppercase tracking-wider text-neutral-500 block mb-1">Sensory Profile</span>
                      <span className="text-sm text-teal-900 font-semibold">{activeModalDish.tastingNotes}</span>
                    </div>
                  </div>
                </div>

                <div className="pt-8 mt-8 border-t border-neutral-900/10 flex items-center justify-between">
                  <span className="text-xs text-neutral-500 italic">Catalogue Inspection Mode</span>
                  <button
                    onClick={() => setActiveModalDish(null)}
                    className="px-6 py-2.5 rounded-full bg-neutral-900 hover:bg-neutral-800 text-teal-100 text-xs font-semibold uppercase tracking-wider transition-colors shadow-lg"
                  >
                    Close Inspection
                  </button>
                </div>
